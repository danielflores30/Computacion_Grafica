...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Model 
		model = glm::rotate(model, glm::radians(hombro), glm::vec3(0.0f, 0.0, 1.0f)); //Se genera la articulaación
		modelTemp = model = glm::translate(model, glm::vec3(1.5f, 0.0f, 0.0f)); //donde movemos el pivote / Generamos la articulación
