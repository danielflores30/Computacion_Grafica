...
float rotDog = 0.0f;
int dogAnim = 0;
float FLegs = 0.0f; //Controlar patas traseras 
float RLegs = 0.0f; //Partes Delanteras 
float head = 0.0f; //Cabezas 
float tail = 0.0f;
...
int main()
{
	...
	//Carga de modelos - Por separados del perro 
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(RLegs), glm::vec3(1.0f, 0.0f, 0.0f)); // Es para que unaas patas vayan hacia delante y otras atras, por eso son positivos y negativos
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); 
		B_LeftLeg.Draw(lightingShader);
		//Back Right Leg
		model = modelTemp; 
		model = glm::translate(model, glm::vec3(-0.083f, -0.057f, -0.231f)); //Valor correspondiente de la cabeza - 
		model = glm::rotate(model, glm::radians(RLegs), glm::vec3(-1.0f, 0.0f, 0.0f)); // Es para que unaas patas vayan hacia delante y otras atras, por eso son positivos y negativos
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void Animation() {
	...
	if (dogAnim == 1) { //animacion caminar
		if (!step) {  //Cuando este en el estado 1 
			...
			if (RLegs > 15.0f) //Condición hasta que llegue a cierto valor 
				step = true;
			}
			else
			{
			...
			if (dogPos.z < 2.3f) { //La Z es hacia la dirección que va el perrito 
				dogPos.z += 0.001f;
			}
			else {
